    ...
    const AN = {
        ...
        post: function( url, data, options ) {
            ...
            $.ajax({
                ...
                async: true,
                cache: false,
                // suppressLoader: behind-the-scenes saves (requests not
                // initiated directly by a user action) opt out of the loading interstitial.
        ...
        // outside form handlers can defer modal
...
function asyncClickHandler(event) {
    ...
    // (e.g. a styled toggle: a <label> wrapping a checkbox + styled
    // <span>) take user clicks on the visible <span> child, which
    // has no other interactive ancestor.
...
function asyncUpdateDataFromJson( $target, $mode, json ) {
    ...
    if ( 'modal' in json ) {
        ...
        // post-modal-insert hook so consumers (e.g., handlers that
        // initialize content inside the modal) can see the modal's DOM in place.
...
// closing long-lived connections (sockets, media streams) bound to
// elements before the browser orphans them.
