...
CONTAINER_NAME="zzz"
EXTERNAL_PORT="9666"
# Resolve the existing install directory. install.sh uses the hidden ~/.zzz by
# default, but falls back to a non-hidden ~/zzz when Docker cannot read
# dot-directories (e.g. snap Docker). Detect by the install's env file (what
# this updater actually needs) rather than a bare directory.
if [[ -f "${HOME}/zzz/env/local.env" ]]; then
    ZZZ_HOME="${HOME}/zzz"
else
    ZZZ_HOME="${HOME}/.zzz"
fi
ENV_FILE="${ZZZ_HOME}/env/local.env"
...
# install.sh-generated compose file exists at ~/.zzz/docker-compose.yml.
...
main() {
    ...
    # install.sh-generated compose file exists. A pre-compose install has no
