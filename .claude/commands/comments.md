...
/comments src/common                      # recurse the directory
/comments src/notify/*.py                  # top-level Python files only
/comments src/notify/templates/            # recurse this whole template tree
/comments src/common/**/*.py               # recursive glob (Python files only, any depth)
```
...
Reviews **only the files matching the glob pattern**. Use this when you want to scope to a slice that a single directory path can't express: top-level files only (`notify/*.py`), a specific file type across siblings, or a deeper selective walk (`common/**/*.py`).

Typical use: a module has shared base files at the top level and feature-specific subdirectories beneath. The natural review cadence is "shared first, then each feature":

```
/comments src/notify/*.py                  # base/shared files
/comments src/notify/templates/            # templates separately
```
...
1. Determine the diff base:
   ...
   - Else if an upstream tracking branch is set (`git rev-parse --abbrev-ref @{u}` succeeds), use `git merge-base HEAD @{u}`.
...
1. Detect glob mode by the presence of glob metacharacters (`*`, `?`, `[`, `]`) in the argument.
...
src/common/history_table_manager.py:88 — kept — Cross-file constant reference;
   could not verify the named constant in the other file.
src/user/views.py:142 — kept — Docstring on what looks like a public API
   method; uncertain whether external callers depend on it.
src/notify/signals.py:201 — removed-uncertain — Said "Mirrors pattern in
   user/signals"; checked and the pattern is no longer there, but flagging in
   case the comment was load-bearing for navigation.
src/common/templates/common/panes/summary.html:14 — kept — HTML comment
   (<!-- -->) that should be {# #}; left content alone, flagging form per
   coding-standards rule.

## Borderline cases worth discussing

1. src/common/email_utils.py:59 — kept — Documents an external service's rate
   limits and retry intervals. Fits the "external service config rationale"
   pattern but was at the edge of being just configuration metadata.

2. src/notify/views.py:88 — removed — "Without this, the reload spams the log"
   felt borderline. Rewrote the surrounding code's intent into the variable
   name instead; comment became redundant.
...
- **Diff base cannot be resolved**: Abort. Suggest `/comments --base origin/staging`.
...
- **Test files are excluded.** The agent does not touch test files in any mode.
