...
# Zzz App - Single Command Installer
# Installs and runs Zzz App with minimal user interaction
# Requires: Docker
...
CONTAINER_NAME="zzz"
EXTERNAL_PORT="9666"
# Install directory. Defaults to the hidden ~/.zzz. choose_install_dir() may
# switch this to the non-hidden ZZZ_HOME_VISIBLE at install time when Docker
...
# from the final ZZZ_HOME by set_install_paths().
ZZZ_HOME="${HOME}/.zzz"
ZZZ_HOME_VISIBLE="${HOME}/zzz"
HAS_COMPOSE=0  # Set by check_docker_compose; 1 if `docker compose` is available.

# Derive all install paths from the (possibly updated) ZZZ_HOME.
set_install_paths() {
    ENV_DIR="${ZZZ_HOME}/env"
    DATABASE_DIR="${ZZZ_HOME}/database"
    MEDIA_DIR="${ZZZ_HOME}/media"
    ENV_FILE="${ENV_DIR}/local.env"
    COMPOSE_FILE="${ZZZ_HOME}/docker-compose.yml"
}
...
print_header() {
    echo -e "${BLUE}================================${NC}"
    echo -e "${BLUE} Zzz App Installer${NC}"
    echo -e "${BLUE}================================${NC}"
}
...
# compose file or --env-file under ~/.zzz and fails with an opaque "permission
...
create_directories() {
    ...
    print_success "Directories created: ${ZZZ_HOME}"
}
...
create_env_file() {
    ...
    # extract the variable names from this exact block without ambiguity. Keep
    # the variable set identical to deploy/env-generate.py --example and
    # local.env.example (deploy/env-drift-check.sh enforces this).
    cat > "${ENV_FILE}" << INSTALL_ENV_FILE_EOF
# Zzz App Environment Configuration
...
# Write a fully-resolved docker-compose.yml to ${ZZZ_HOME}/. Always written, even
# when compose is not available, so it is ready for use the moment the user
# installs the compose plugin. Container name `zzz` matches the legacy
# `docker run --name zzz` invocation so management commands like `docker logs
# zzz`, `docker stop zzz`, and `docker start zzz` work identically across both
...
# Zzz App — generated by install.sh
#
...
# Pull Docker image
pull_docker_image() {
    print_info "Pulling Zzz App Docker image..."
...
# paths produce a container named `zzz`, so post-install management commands
# documented in show_success() work identically.
start_container() {
    print_info "Starting Zzz App container..."
...
wait_for_app() {
    ...
    print_warning "Application may still be starting. Check docker logs ${CONTAINER_NAME} for details."
}
...
show_success() {
    ...
    echo -e "${GREEN}================================${NC}"
    echo
    echo -e "${BLUE}🌐 Access your Zzz App system:${NC}"
