...
def requires_api_key(self) -> bool:
    """This provider's public API is unauthenticated."""
    return False
```

The docstring tells the reader *why* (the provider's actual policy), not just *what* (returns False). Without it, the reader infers the policy by absence -- workable but worse than knowing.
...
- A higher-level docstring (class, module) already establishes the WHY. For example, a manager class that documents its robust-loading strategy at the class level makes per-method `"""Create X with error tracking..."""` docstrings redundant -- the WHY lives once at the class level.
- The docstring restates only the WHAT (`"""Return False."""`, `"""Returns the id."""`).
...
if item_id not in lookup:
    continue  # Skip if item creation failed upstream
```
...
- **Parallel-module archeology**: "Like the X module..." / "Same shape as the sibling Y..." / "(same lifecycle as `_fault_mode` on the base class)" / "Same approach Z uses for this." Recurs heavily when a new module is written by mirroring a sister module -- the parallel is real *for the author*, but the cold reader only needs the current file's contract, not a pointer to chase the sister file. Strip the cross-reference; keep any standalone substance the surrounding sentence carries.
...
- **Issue number references** (`(Issue #281)`, `(#263)`, `Per issue #N`). The issue tracker holds the historical record; the docstring is the future contract. The issue number is workflow metadata, not code documentation.
...
- **Caller-name lists**: `"Used by the disable flow and by the refresh path"`. The list rots whenever a caller is renamed, added, or removed.
- **"for use from X" tails**: `"Async variant of get_entry for use from async monitor paths"` -- the `async def` already says "from async contexts"; the specific caller enumeration adds nothing.
- **Sibling cross-references**: `"Parallel to get_connector() and get_importer()"` -- once a reader has scanned the methods, the parallelism is self-evident. The reference adds maintenance burden without information.
...
- Modal section references: `"the collapsed Details section"`, `"the sidebar list"`.
...
**Carve-out: external API documentation URLs are keep-worthy.** A URL to the definitive contract documentation for an external system the code depends on (a vendor's published OpenAPI spec, an RFC, a protocol specification) is a bookmark to the source of truth -- not a maintenance liability. If the URL rots, the implementation that depends on the underlying API has a bigger problem than the comment.
...
Examples:
- `"Phase 5: the confirm action on the preview modal posts to the run view."` -> strip `"Phase 5: "`, residue describes a real workflow -> REWRITE (keep the workflow description).
- `"Issue #283 retry probe rides on the client."` -> strip `"Issue #283 "`, residue is informational about the client -> REWRITE.
...
Django view classes (`ModalView`, `View`, and page-view subclasses, etc.) sit at the boundary between backend logic and rendered UI. The same UI-label exception that applies to templates and CSS applies here: the view is the file that *defines* the rendering for the UI surface it owns.

**UI labels that the view directly renders, dispatches on, or names in its modal/page contract are keep-worthy.** Examples that earn their place:
...
- CTAs and modal flows the view assembles: `'Place N new items' CTA`, "the manage page", "the result modal".
...
Pattern #4 still applies when a backend non-view file references UI labels owned elsewhere (e.g., a manager class mentioning a specific tile). The view-layer carve-out is for the view itself.
...
These strings may contain content that would otherwise match remove-on-sight patterns (specific UI references, example values). The audience justifies them; leave them alone. If in doubt about whether a string is user-facing, leave it.

### ASCII Normalization

The cleanup pass also normalizes non-ASCII characters in comments and docstrings to ASCII equivalents per [Coding Standards](coding-standards.md). This is a syntactic concern but is applied during the cleanup pass for convenience. Common substitutions: em/en dash -> `--`, right arrow -> `->`, ellipsis -> `...`, multiplication sign -> `x`, degree symbol -> drop or restructure.
