...
. ./dev/init-env-dev.sh
pip install -r src/zzz/requirements/development.txt
cd src
./manage.py check
./manage.py migrate
./manage.py bootstrap
...
Then, visit: [http://127.0.0.1:8666](http://127.0.0.1:8666) to access the app.
...
./manage.py test zzz.my_module      # Run specific app tests

# User management
./manage.py bootstrap

# Development server
./manage.py runserver              # Runs on http://127.0.0.1:8666
```
